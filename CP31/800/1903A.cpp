...
int main()
{
    ...
    while(n--){
        ...
        if(k>1){
            cout<<"YES\n";//if k =1, the array cannot be reversed and hence cannot be sorted(if its not sorted already)
        }
        else{
            if(is_sorted(boxes.begin(),boxes.end())){
                cout<<"YES\n";//if already sorted, then we can say YEs
            }
            else{
                cout<<"NO\n";//if not sorted, then we can say NO
            }
        }
    }
